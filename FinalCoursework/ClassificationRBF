...
for i = 1:10
    ...
    %test the model
    currentModel = fitcsvm(trainingInputs,trainingTargets, 'Standardize',true,'KernelFunction', 'polynomial', 'PolynomialOrder',currentModelParameters.sigmaValue, 'BoxConstraint',currentModelParameters.cValue); 
...
SVMPolynomialPredictions = SVMPolynomialPredictionsMat(:)
...
function [bestHyperParameter] = innerLoop(inputs,targets,k)
    ...
    %sigma values
    sigmaValues = [0.01, 0.05,  1 , 5 , 10, 50, 100, 500, 1000, 5000, 10000 ];
