...
parfor i = 1:10
    ...
    currentModel = fitrsvm( trainingInputs, trainingTargets, 'Standardize', true, 'KernelFunction', 'linear', 'BoxConstraint', cValue, 'Epsilon', epsilonValue);
    
    display("c values = " + cValue);

    display("epsilon values = " + epsilonValue);

    display("finish " + i + "-fold training SVM: " + toc + " sec");
...
%                     SVM = fitrsvm( trainingInputs, trainingTargets, 'Standardize', true, 'KernelFunction', 'rbf', 'KernelScale', sigmaValues(sigmaIndex), 'BoxConstraint', cValues(cIndex), 'Epsilon', epsilonValues(eIndex));
                    ...
                    display("finish " + i + "-fold training SVM: " + toc + " sec");
                    tPrint("finish " + i + "-fold training SVM: " + toc + " sec");
...
function [trainingInputs, trainingTargets, testingInputs, testingTargets] = myCVPartition(foldLength, numOfExamples, i, P, k, inputs, targets)
    ...
    %disp("foldLength = " + foldLength);
    %disp("validPerm = " + validPerm);
